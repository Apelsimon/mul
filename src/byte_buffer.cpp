...
size_t byte_buffer::get_read_available() const
{
    if(!invariant())
    {
        throw std::runtime_error{"Invariant violated"};
...
size_t byte_buffer::get_write_available() const
{
    if(!invariant())
    {
        throw std::runtime_error{"Invariant violated"};
...
bool byte_buffer::operator==(const byte_buffer& other) const
{
    ...
    for(size_t i = 0; i < get_read_available(); ++i)
    {
...
std::ostream& operator<<(std::ostream& os, const mul::byte_buffer& buff)
{
    buff.print(os);
    return os;
}
